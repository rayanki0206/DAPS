...
locals {
  ...
  dl_container_scope = "/subscriptions/${data.azurerm_subscription.dapp.subscription_id}/resourceGroups/${local.dlsa_rg_name}/providers/Microsoft.Storage/storageAccounts/${var.existing_datalake}/blobServices/default/containers/${local.dl_container}"
    dlsa = data.azurerm_resources.storageaccounts
  valid_dlsa = {
    for resource in data.azurerm_resources.storageaccounts.resources : resource.name => resource
    if contains(tolist([var.existing_datalake]), resource.name)
  }
  dlsa_rg = local.valid_dlsa
  dlsa_rg_name = values(local.dlsa_rg)[0].resource_group_name


  datafactory = data.azurerm_resources.df.resources
  valid_df = {
    for res in local.datafactory: res.name => res
    if contains(tolist([var.data_factory_name]),res.name)
  }
  df = local.valid_df
